...
int get_port_lower_layer(char *refparam, struct dmctx *ctx, char **value)
{
	...
	if (ifname[0] != '\0' && strcmp(mg_port, "true") ==  0) {
		...
		for (pch = strtok_r(ifname_dup, " ", &spch); pch != NULL; pch = strtok_r(NULL, " ", &spch)) {
			...
			adm_entry_get_linker_param(ctx, dm_print_path(DMROOT"%cBridging%cBridge%c", dm_delim, dm_delim, dm_delim), plinker, value);
			if (*value == NULL)
				*value = "";
...
int get_vlan_port_vlan_ref(char *refparam, struct dmctx *ctx, char **value)
{
	char linker[8];
	char *name;
	dmasprintf(&name,DMROOT"%cBridging%cBridge%c%s%c", dm_delim, dm_delim, dm_delim, cur_bridging_args.br_key, dm_delim);
	sprintf(linker,"vlan%s_%s", cur_bridging_vlan_args.vlan_port, cur_bridging_args.br_key);
	adm_entry_get_linker_param(ctx, dm_print_path(DMROOT"%cBridging%cBridge%c", dm_delim, dm_delim, dm_delim), linker, value); // MEM WILL BE FREED IN DMMEMCLEAN
	if (*value == NULL)
		*value = "";
	return 0;
}
