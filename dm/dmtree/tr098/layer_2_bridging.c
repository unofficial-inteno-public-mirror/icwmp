	...
	for (pch = strtok_r(ifname_tmp, " ", &spch); pch != NULL; pch = strtok_r(NULL, " ", &spch)) {
		if (strncmp(pch, wan_baseifname, 4) == 0
			...
			uci_foreach_option_eq("layer2_interface_vlan", "vlan_interface", "ifname", pch, s) {
				vid = strchr(pch, '.') + 1;
				update_bridge_vlan_config(vid, cur_args.bridge_instance);
				break;
			}
		}
	}
...
int delete_layer2bridging_marking(struct dmctx *ctx, unsigned char del_action)
{
...
int delete_layer2bridging_bridge_vlan(struct dmctx *ctx, unsigned char del_action)
{
	char *vid, *ifname;
	char new_ifname[128];
...
inline int browselayer2_availableinterfaceInst(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
	...
	for (i=0; i<3; i++) {
		uci_foreach_sections(wan_interface_tab[i].package, wan_interface_tab[i].section, wan_s) {
			...
			dmasprintf(&oface, DMROOT"%cWANDevice%c%s%cWANConnectionDevice%c%s%c", dm_delim, dm_delim, wan_interface_tab[i].instance, dm_delim, dm_delim, waninstance, dm_delim); // MEM WILL BE FREED IN DMMEMCLEAN
	...
	while (ch_ptr != NULL)
	{
		dmasprintf(&oface, DMROOT"%cLANInterfaces%cLANEthernetInterfaceConfig%c%d%c", dm_delim, dm_delim, dm_delim, ++i, dm_delim); // MEM WILL BE FREED IN DMMEMCLEAN
	...
	uci_foreach_sections("wireless", "wifi-iface", wifi_s) {
		dmasprintf(&oface, DMROOT"%cLANInterfaces%cWLANConfiguration%c%d%c", dm_delim, dm_delim, dm_delim, ++i, dm_delim); // MEM WILL BE FREED IN DMMEMCLEAN
