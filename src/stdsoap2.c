...
{ register struct soap_nlist *np;
  ...
  n = strlen(id);
  k = strlen(ns) + 1;
  /* KMD Support both spec versions urn:dslforum-org:cwmp-1-0 and urn:dslforum-org:cwmp-1-1*/
...
soap_element(struct soap *soap, const char *tag, int id, const char *type)
{
  ...
  else
  {
    ...
    { if (!(soap->mode & SOAP_XML_CANONICAL)
       && soap_send(soap, NULL))/* TODO KMD to avoid the put of xml tag in the soap messages */
        return soap->error;
    }
...
{ soap_wchar c;
  ...
  *soap->endpoint = '\0';
  ...
  { soap_mode m = soap->imode;
    ...
    if (soap->error == 401) /* KMD ADDED: avoid close socket when receive 401 and Connection != Close*/
    {
    	return soap->error;
    }
...
{ char header[SOAP_HDRLEN], *s;
  ...
  *soap->endpoint = '\0';
  ...
  /* Status 201 (Created), 202 (Accepted), ... and HTTP 400 and 500 errors
     require normal return to try parsing SOAP/XML body.
     Otherwise, try to parse HTTP body.
  */
  if(k==401 && soap->keep_alive!=0) /* KMD ADDED: avoid close socket when receive 401 and Connection != Close*/
    return soap->error = k;
...
{ register const char *s;
  ...
  if ((err = soap->fposthdr(soap, "Host", soap->tmpbuf))
   || (err = soap->fposthdr(soap, "User-Agent", "INTENO CWMP"))
...
{ register int err;
  ...
  if ((err = soap->fposthdr(soap, "Server", "INTENO CWMP"))
